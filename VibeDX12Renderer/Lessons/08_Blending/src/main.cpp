...
class BlendingApp : public VibeDX12App {
public:
    BlendingApp(HINSTANCE hInstance) : VibeDX12App(hInstance) {
        m_MainWndCaption = L"Lesson 08: Blending (Transparency)";
        m_camera.SetPosition(0.0f, 2.0f, -7.0f);
    }
...
private:
    ...
    void BuildPSOs(); // Opaque and Transparent PSOs
...
LRESULT BlendingApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch(msg) {
        case WM_LBUTTONDOWN:
        case WM_MBUTTONDOWN:
        case WM_RBUTTONDOWN:
...
void BlendingApp::OnKeyboardInput() {
    float dt = 0.016f; 
    float speed = 5.0f; 
...
void BlendingApp::OnUpdate() {
    ...
    m_constants.AmbientLight = { 0.25f, 0.25f, 0.35f, 1.0f }; // Slightly blue ambient
}

void BlendingApp::BuildRenderItems() {
    // 1. Opaque Grid
    RenderItem grid;
    XMStoreFloat4x4(&grid.World, XMMatrixTranspose(XMMatrixTranslation(0.0f, -1.0f, 0.0f)));
    grid.DiffuseAlbedo = { 0.5f, 0.5f, 0.5f, 1.0f }; // Opaque
    grid.FresnelR0 = { 0.02f, 0.02f, 0.02f };
    grid.Roughness = 0.8f;
    grid.IndexCount = 6;
    grid.StartIndexLocation = 20 * 20 * 6; // Grid starts after sphere
    grid.BaseVertexLocation = (20 + 1) * (20 + 1);
    m_opaqueItems.push_back(grid);

    // 2. Opaque Sphere (Center)
    RenderItem sphere;
    XMStoreFloat4x4(&sphere.World, XMMatrixTranspose(XMMatrixTranslation(0.0f, 0.0f, 0.0f)));
    sphere.DiffuseAlbedo = { 0.8f, 0.2f, 0.2f, 1.0f }; // Opaque Red
    sphere.FresnelR0 = { 0.05f, 0.05f, 0.05f };
    sphere.Roughness = 0.1f;
    sphere.IndexCount = 20 * 20 * 6;
    sphere.StartIndexLocation = 0;
    sphere.BaseVertexLocation = 0;
    m_opaqueItems.push_back(sphere);

    // 3. Transparent Water Cube (Using scaled Grid geometry for simplicity, or we can just render the grid mesh raised up?)
    // Let's reuse the Grid geometry but raised up to act as a "water surface" or "Glass plane"
    RenderItem water;
    XMStoreFloat4x4(&water.World, XMMatrixTranspose(XMMatrixTranslation(0.0f, 0.5f, 0.0f))); 
    water.DiffuseAlbedo = { 0.0f, 0.2f, 1.0f, 0.5f }; // Blue 50% Alpha
    water.FresnelR0 = { 0.1f, 0.1f, 0.1f };
    water.Roughness = 0.0f;
    water.IndexCount = 6;
    water.StartIndexLocation = 20 * 20 * 6;
    water.BaseVertexLocation = (20 + 1) * (20 + 1);
    
    // Transparent items should be sorted back-to-front. 
    // Here we only have one, so trivial.
    m_transparentItems.push_back(water);
}

void BlendingApp::BuildRootSignature() {
    // Just b0 for constants
...
void BlendingApp::BuildGeometry() {
    // Reusing the Sphere + Grid geometry generation style
    ...
    float depth = 20.0f;
    Vertex v[4];
    v[0] = { {-width*0.5f, 0.0f, -depth*0.5f}, {0.0f, 1.0f, 0.0f} };
    v[1] = { {-width*0.5f, 0.0f, +depth*0.5f}, {0.0f, 1.0f, 0.0f} };
    v[2] = { {+width*0.5f, 0.0f, +depth*0.5f}, {0.0f, 1.0f, 0.0f} };
    v[3] = { {+width*0.5f, 0.0f, -depth*0.5f}, {0.0f, 1.0f, 0.0f} };
...
void BlendingApp::BuildPSOs() {
    ...
    // Transparent objects generally do not write to depth buffer (unless doing alpha testing/discard)
    // because the background needs to be seen through them. But standard transparency: read depth, don't write.
    // However, if we sort correctly, we can write OR disable write. 
    // Standard approach for simple transparency: disable depth write.
    psoDesc.DepthStencilState.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;

    ThrowIfFailed(m_d3dDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_psoTransparent)));
}

void BlendingApp::OnRender() {
    ThrowIfFailed(m_commandAllocator->Reset());
    // Start with Opaque PSO
    ...
    // Clear to a generic background color
    const float clearColor[] = { 0.6f, 0.8f, 0.9f, 1.0f }; // Sky Blue
    ...
    // 2. Switch to Transparent PSO
    m_commandList->SetPipelineState(m_psoTransparent.Get());

    // 3. Draw Transparent Items (sorted back to front ideally)
    for(auto& ri : m_transparentItems) {
        m_constants.World = ri.World;
        m_constants.DiffuseAlbedo = ri.DiffuseAlbedo;
        m_constants.FresnelR0 = ri.FresnelR0;
        m_constants.Roughness = ri.Roughness;
