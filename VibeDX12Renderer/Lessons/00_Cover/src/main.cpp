...
class CoverApp : public VibeDX12App {
public:
    CoverApp(HINSTANCE hInstance) : VibeDX12App(hInstance) {
        m_MainWndCaption = L"Vibe Graphics - Project Cover";
        // Override window size if needed, but default 1280x720 is fine for screenshot
    }
...
void CoverApp::OnUpdate() {
    static float theta = 0.0f;
    theta -= 0.005f;

    // Diagonal rotation axis (1, 1, 1) normalized
    XMVECTOR rotationAxis = XMVector3Normalize(XMVectorSet(1.0f, 0.5f, 0.5f, 0.0f));
    XMMATRIX world = XMMatrixScaling(0.7f, 0.7f, 0.7f) * XMMatrixRotationAxis(rotationAxis, theta);

    // Fixed camera
    XMVECTOR pos = XMVectorSet(0.0f, 0.0f, -3.5f, 1.0f);
    ...
    XMStoreFloat4x4(&m_worldViewProj, XMMatrixTranspose(wvp));
}

void CoverApp::OnRender() {
    ...
    // Dark Gray Background for cover
    float darkGray[] = { 0.15f, 0.15f, 0.15f, 1.0f };
    m_commandList->ClearRenderTargetView(rtv, darkGray, 0, nullptr);
    ...
    ThrowIfFailed(m_swapChain->Present(1, 0)); // Sync with VSync
    m_currBackBuffer = (m_currBackBuffer + 1) % SwapChainBufferCount;
    FlushCommandQueue();
}

void CoverApp::BuildRootSignature() {
    ...
    if(errorBlob != nullptr) {
        MessageBoxA(nullptr, (char*)errorBlob->GetBufferPointer(), "RootSig Warning", MB_OK);
    }
    ThrowIfFailed(hr);
...
void CoverApp::BuildShadersAndInputLayout() {
    UINT compileFlags = 0;
#if defined(_DEBUG)
    ...
    if(errors != nullptr) {
        MessageBoxA(nullptr, (char*)errors->GetBufferPointer(), "VS Compile Warning/Error", MB_OK);
    }
    ...
    if(FAILED(hr)) {
        ...
        std::filesystem::path shaderPath = exePath.parent_path() / "shaders.hlsl";
        
        hr = D3DCompileFromFile(shaderPath.c_str(), nullptr, nullptr, "VS", "vs_5_0", compileFlags, 0, &m_vsByteCode, &errors);
        if(errors != nullptr) {
             MessageBoxA(nullptr, (char*)errors->GetBufferPointer(), "VS Compile Retry Error", MB_OK);
        }
    }
    ThrowIfFailed(hr);
    ...
    if(errors != nullptr) {
         MessageBoxA(nullptr, (char*)errors->GetBufferPointer(), "PS Compile Warning/Error", MB_OK);
    }
    if(FAILED(hr)) {
        wchar_t buf[MAX_PATH];
        GetModuleFileName(nullptr, buf, MAX_PATH);
        ...
        hr = D3DCompileFromFile(shaderPath.c_str(), nullptr, nullptr, "PS", "ps_5_0", compileFlags, 0, &m_psByteCode, &errors);
        if(errors != nullptr) {
            MessageBoxA(nullptr, (char*)errors->GetBufferPointer(), "PS Compile Retry Error", MB_OK);
        }
    }
    ThrowIfFailed(hr);
...
void CoverApp::BuildTexture() {
    ...
    if (!img) {
        ...
        GetModuleFileName(nullptr, buf, MAX_PATH);
        std::filesystem::path exePath = buf;
        std::filesystem::path assetPath = exePath.parent_path() / "Assets" / "512x512_Texel_Density_Texture_1.png";
        img = stbi_load(assetPath.string().c_str(), &width, &height, &channels, 4);
    }

    if (!img) {
        throw std::runtime_error("Failed to load texture for cover. Checked relative paths and next to exe.");
    }
    ...
    // FIX: Do NOT Reset here! The command list is already open in Initialize().
    // ThrowIfFailed(m_commandList->Reset(m_commandAllocator.Get(), nullptr));
    ...
    // FIX: Do NOT Close/Execute here! Let Initialize() handle submission.
    /*
    ThrowIfFailed(m_commandList->Close());
    ID3D12CommandList* cmdsLists[] = { m_commandList.Get() };
    m_commandQueue->ExecuteCommandLists(1, cmdsLists);
    
    FlushCommandQueue(); // Wait for upload
    */
}

void CoverApp::BuildDescriptorHeaps() {
